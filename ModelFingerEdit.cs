...
public class ModelFingerEdit : MonoBehaviour
{
    ...
    [SerializeField] private /*const*/ float PanDirectionThreshold = 0.1f;
    ...
    private void OnTwoTouchesStay(Vector2 old0, Vector2 old1, Vector2 new0, Vector2 new1)
    {
        ...
        // if (ApplyZoom(old0, old1, new0, new1)) { }
    }
    ...
    {
        Vector2 oldDir = old0 - old1;
        Vector2 newDir = new0 - new1;

        if(Vector2.Angle(oldDir, newDir) > PanDirectionThreshold)
            return false;
    ...
    private bool ApplyZoom(Vector2 old0, Vector2 old1, Vector2 new0, Vector2 new1)
    {
        ...
        if (scaleCalculation != 0)
            Debug.Log($"rotationAngle: {scaleCalculation}");
